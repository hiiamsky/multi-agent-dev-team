...
- **狀態**：提案中
- **日期**：2026-05-01
- **提出者**：人類 + Claude（依 `agent-system-refactor-plan-final.md`）
...
依 `agent-system-refactor-plan-final.md` 執行 Phase 0–5 重構，目標是讓每個 agent 在更少 context、更少歧義、更清楚成功標準下工作。本 ADR 為重構決策的權威紀錄，後續每個 Phase 完成後在此追加進度註記。
...
具體執行內容：
- **Phase 0**：本 ADR + 基線量測（`refactor/docs/baseline.md`）。
...
| 採用 `agent-system-refactor-plan-karpathy.md` 的激進路徑（廢除 Orchestrator/QA/QC、移除 Handoff Contract、移除 human-decision-protocol、AGENTS.md ≤50 行） | 經逐條對照 Karpathy 原文後**整體符合度 4.5/10**：原文僅支持 2 條（Anti-Swarm hype、Declarative Leverage），其餘 5 條為**超譯**或**邏輯反向**（例：原文「LLM 不會主動釐清」被誤推為「移除 human-decision-protocol」）。且與企業多 agent + 法遵情境的合規承載不符。完整評估見 `agent-system-refactor-plan-final.md` §0 |
...
- **規則 1**：本重構期間，原 `.github/` 維持運作不動；所有重構產出寫入 `refactor/.github/`、`refactor/docs/`。
...
- 整合計畫：`agent-system-refactor-plan-final.md`
- 已評估替代：`agent-system-refactor-plan-karpathy.md`（不採用，符合度 4.5/10）
- 原始來源核對：`docs/Andrej Karpathy.md`
- Issue: 待開
- PR: 待開
...
| Phase 1 | 完成 | 2026-05-01 | `AGENTS.md` 瘦身產出於 `refactor/.github/AGENTS.md` |
| Phase 2 | 完成 | 2026-05-01 | 3 個 SSOT skill 建立於 `refactor/.github/skills/` |
| Phase 3 | 完成 | 2026-05-01 | 7 個 agent 檔案重寫於 `refactor/.github/agents/` |
| Phase 4 | 完成 | 2026-05-01 | `agent-handoff-contract` skill v2 於 `refactor/.github/skills/` |
