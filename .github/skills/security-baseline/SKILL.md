...
| SA/SD | owasp-web-top10.md §威脅建模、owasp-api-top10.md、owasp-llm-top10.md、pdpa-compliance.md | supply-chain-tooling.md、**threat-model-analyst**（新系統 / 重大架構變更時） |
...
- **需求淨化階段(Orchestrator)** → 依安全標籤判斷是否觸發下游安全設計:
  ...
  - 涉及 AI / LLM 功能 → SA/SD 載入 owasp-llm-top10.md (全部 10 項)
  - 涉及第三方依賴變更 → SA/SD 載入 supply-chain-tooling.md

- **架構設計階段(SA/SD)** → 產出安全設計章節,引用本 skill 對應條目
  - 新系統 / 重大架構變更 / 安全標籤同時勾選三項以上 → **先執行 `/threat-model-analyst`**，以 STRIDE-A 分析作為安全設計輸入
  - 功能迭代（已有歷史報告）→ 執行 `/threat-model-analyst` **Incremental 模式**
...
> 🔍 **深度威脅建模工具**：`.github/skills/threat-model-analyst/` — 執行完整 STRIDE-A 威脅模型或增量差異分析（新系統 / 重大架構變更時由 SA/SD 呼叫 `/threat-model-analyst`）。此 skill 來自 [github/awesome-copilot](https://github.com/github/awesome-copilot/tree/main/skills/threat-model-analyst)，與本 skill 定位不同：本 skill 是知識庫（規則 + 檢查清單），threat-model-analyst 是執行工具（主動分析 + 報告產出）。
...
| AI / LLM 標籤勾選,但藍圖缺少 Prompt Injection 緩解設計 | SA/SD | Critical |
| AI / LLM 標籤勾選,但實作未驗證 LLM 輸出 (直接 eval / 渲染) | 後端 PG / 前端 PG | Critical |
| AI / LLM 標籤勾選,但 system prompt 含硬編碼 API Key / 密碼 | SA/SD (prompt 設計者) | Critical |
| 新增 agent 或擴展工具白名單,但未經 LLM06 Excessive Agency 審查 | Orchestrator + SA/SD | High |
