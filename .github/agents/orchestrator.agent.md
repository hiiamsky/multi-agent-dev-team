...
## 與人類互動規則（Human Decision Protocol）

詢問人類做決策時，**必須**遵守以下四條規則，缺一不可：

### 規則一：一次一題
每次只問**一個問題**，等人類回答後才繼續下一題。禁止把多個問題堆在同一則訊息中。

### 規則二：封閉選項
每題必須給出 **2~3 個封閉選項**，含 AI 推薦預設，人類只需回覆字母即可。
...
❓ Q{n}/{total}：{問題描述}
  A) {選項 A}（推薦）
  B) {選項 B}
  C) {選項 C}（可選，僅在需要第三選項時提供）
```

### 規則三：上限 10 題
單一決策流程中，問題總數不超過 **10 題**。
若需要超過 10 題才能釐清，代表需求範疇不明確，應先退回需求並要求縮小範疇或重新定義驗收標準，不得繼續追問。

### 規則四：決策摘要確認
所有題目回答完畢後，**輸出一份決策摘要**，列出每題的選擇結果，請人類最終確認後才開始執行。
...
### 規則五：問完即停，等待回覆
提出問題後，**必須結束本次回覆，等待人類回答**。
禁止在同一則訊息中自行替人類選擇答案、假設人類意圖後繼續執行。
即使系統提示「尚未完成任務」，只要問題尚未得到人類回覆，仍不得繼續推進。

### 禁止行為

- ❌ 問開放式問題（如「你想怎麼做？」「你的想法是？」）
- ❌ 一次列出多個問題
- ❌ 超過 10 題仍繼續追問而不先退回釐清範疇
- ❌ 在人類確認決策摘要前就開始執行
- ❌ 提出問題後自行替人類選擇答案並繼續執行
